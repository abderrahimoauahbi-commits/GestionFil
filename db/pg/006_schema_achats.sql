...
CREATE TABLE bon_commande (
    ...
    date_bc             text    NOT NULL DEFAULT to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
    ...
    taux_change_engage  numeric(9,4)    NOT NULL CHECK (taux_change_engage > 0),
    date_taux_engage    text,

    montant_total_devise numeric(18,2)   CHECK (montant_total_devise IS NULL OR montant_total_devise >= 0),
    montant_total_mad   numeric(18,2)    CHECK (montant_total_mad IS NULL OR montant_total_mad >= 0),
...
CREATE TABLE ligne_bc (
    ...
    besoin_kg_origine   numeric(18,4)    CHECK (besoin_kg_origine IS NULL OR besoin_kg_origine >= 0),
...
CREATE TABLE plan_achat (
    id_proposition      text    NOT NULL PRIMARY KEY
                                DEFAULT gen_random_uuid()::text,
    date_generation     text    NOT NULL DEFAULT to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
    id_plan             text    REFERENCES plan_production(id_plan),
    code_reference      text    NOT NULL REFERENCES reference(code_reference),

    quantite_suggeree_kg numeric(18,4)   NOT NULL CHECK (quantite_suggeree_kg > 0),
    ...
    quantite_mrp_kg     numeric(18,4)    CHECK (quantite_mrp_kg IS NULL OR quantite_mrp_kg > 0),
...
CREATE TABLE historique_prix (
    ...
    conditions          text,
    date_enregistrement text    NOT NULL DEFAULT to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
