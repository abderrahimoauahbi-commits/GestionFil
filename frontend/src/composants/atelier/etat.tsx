...
const DYNAMIQUES: {
  ...
  {
    motif: /^\/etats\/qualite\/([^/]+)$/,
    decrire: (m) => ({ titre: `Fiche ${m[1]}`, detail: 'Impression', Icone: FileText }),
  },
  {
    motif: /^\/etats\/([a-z-]+)\/[^/]+$/,
    decrire: (m) => ({ titre: `Etat ${m[1].replace('-', ' ')}`, detail: 'Impression', Icone: FileText }),
  },
  {
  ...
  },
  {
    ...
    decrire: () => ({ titre: 'Nouvelle réception import', Icone: Package }),
  },
  {
